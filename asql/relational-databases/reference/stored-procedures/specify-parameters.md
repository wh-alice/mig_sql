...
> [!WARNING]  
>  Any parameter passed in the form **@parameter =***value* with the parameter misspelled, will cause [!INCLUDE[ssNoVersion](../../../advanced-analytics/r-services/includes/ssnoversion-md.md)] to generate an error and prevent procedure execution.  
...
## Specifying Parameter Direction  
 ...
 The following example creates the `Production.usp`_`GetList` procedure, which returns a list of products that have prices that do not exceed a specified amount. The example shows using multiple SELECT statements and multiple OUTPUT parameters. OUTPUT parameters allow an external procedure, a batch, or more than one [!INCLUDE[tsql](../../../advanced-analytics/r-services/includes/tsql-md.md)] statement to access a value set during the procedure execution.  
...
```  
 ...
 Execute `usp_GetList` to return a list of [!INCLUDE[ssSampleDBCoShort](../../../analysis-services/data-mining/includes/sssampledbcoshort-md.md)] products (Bikes) that cost less than $700. The OUTPUT parameters **@cost** and **@compareprices** are used with control-of-flow language to return a message in the **Messages** window.  
