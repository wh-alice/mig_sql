...
### The Data File  
 ...
 When a row is deleted or updated, the row is not removed or changed in-place in the data file but the deleted rows are tracked in another type of file: the delta file. Update operations are processed as a tuple of delete and insert operations for each row. This eliminates random IO on the data file.  
   ...
   Size: Each data file is sized approximately to 128MB for computers with memory greater than 16GB, and 16MB for computers with less than or equal to 16GB. In [!INCLUDE[ssSQL15](../../a9notintoc/includes/sssql15-md.md)] SQL Server can use large checkpoint mode if it deems the storage subsystem is fast enough. In large checkpoint mode, data files are sized at 1GB. This allows for greater efficiency in the storage subsystem for high-throughput workloads.  
...
## Accessing Data and Delta Files  
 ...
 This thread appends inserts and deletes to memory-optimized data rows, to the corresponding data and delta file pairs. In [!INCLUDE[ssSQL14](../../a9notintoc/includes/sssql14-md.md)] there is one offline checkpoint worker; starting [!INCLUDE[ssSQL15](../../a9notintoc/includes/sssql15-md.md)] there are multiple checkpoint workers.  
...
### [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)] Merge Policy  
 [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)] implements the following merge policy:  
