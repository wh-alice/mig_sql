...
## Remarks  
 sp_getbindtoken will return a valid token only when the stored procedure is executed inside an active transaction. Otherwise, the [!INCLUDE[ssDE](../../../analysis-services/instances/install/windows/includes/ssde-md.md)] will return an error message. For example:  
...
```  
 ...
 When sp_getbindtoken is used to enlist a distributed transaction connection inside an open transaction, [!INCLUDE[ssNoVersion](../../../advanced-analytics/r-services/includes/ssnoversion-md.md)] returns the same token. For example:  
...
```  
 ...
 The bind token can be used with sp_bindsession to bind new sessions to the same transaction. The bind token is only valid locally inside each instance of the [!INCLUDE[ssDE](../../../analysis-services/instances/install/windows/includes/ssde-md.md)] and cannot be shared across multiple instances.  
