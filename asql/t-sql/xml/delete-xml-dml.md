...
## Arguments  
 *Expression*  
 Is an XQuery expression identifying the nodes to be deleted. All the nodes selected by the expression, and also all the nodes or values that are contained within the selected nodes, are deleted. As described in [insert (XML DML)](../../t-sql/data-types/insert-xml-dml.md), this must be a reference to an existing node in the document. It cannot be a constructed node. The expression cannot be the root (/) node. If the expression returns an empty sequence, no deletion occurs and no errors are returned.  
...
-   The [modify() Method (xml Data Type)](../../t-sql/data-types/modify-method-xml-data-type.md) is used to specify the **delete** XML DML keyword.  
  
-   The [query() Method (xml Data Type)](../../t-sql/data-types/query-method-xml-data-type.md) is used to query the document.  
