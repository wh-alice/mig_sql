...
-   The total size of the index key, in [!INCLUDE[ssDEnoversion](../../a9notintoc/includes/ssdenoversion-md.md)], cannot exceed 900 bytes. [!INCLUDE[ssDEnoversion](../../a9notintoc/includes/ssdenoversion-md.md)] will assert this condition based on the fixed length key columns when the CREATE INDEX statement is processed. However, if there are variable length columns in the index key, [!INCLUDE[ssDEnoversion](../../a9notintoc/includes/ssdenoversion-md.md)] will also assert this condition for every update to the base tables. Because different aggregations have different view definitions, ROLAP processing using indexed views can succeed or fail depending on the aggregation design.  
  
-   The session creating the indexed view must have the following options set to ON: ARITHABORT, CONCAT_NULL_YEILDS_NULL, QUOTED_IDENTIFIER, ANSI_NULLS, ANSI_PADDING, and ANSI_WARNING. This setting can be made in [!INCLUDE[ssManStudioFull](../../a9notintoc/includes/ssmanstudiofull-md.md)].  
  
-   The session creating the indexed view must have the following option set to OFF: NUMERIC_ROUNDABORT. This setting can be made in [!INCLUDE[ssManStudioFull](../../a9notintoc/includes/ssmanstudiofull-md.md)].  
