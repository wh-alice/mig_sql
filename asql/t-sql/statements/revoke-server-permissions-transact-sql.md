...
### A. Revoking a permission from a login  
 The following example revokes `VIEW SERVER STATE` permission from the [!INCLUDE[ssNoVersion](../../advanced-analytics/r-services/includes/ssnoversion-md.md)] login `WanidaBenshoof`.  
...
### B. Revoking the WITH GRANT option  
 The following example revokes the right to grant `CONNECT SQL` from the [!INCLUDE[ssNoVersion](../../advanced-analytics/r-services/includes/ssnoversion-md.md)] login `JanethEsteves`.  
