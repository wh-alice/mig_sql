...
[!INCLUDE[tsql-appliesto-ss2016-asdb-xxxx-xxx_md](../../a9notintoc/includes/tsql-appliesto-ss2016-asdb-xxxx-xxx-md.md)]
  ...
  In-Memory OLTP introduces memory-optimized tables and natively compiled stored procedures in [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)]. This article gives an overview of query processing for both memory-optimized tables and natively compiled stored procedures.  
  
 The document explains how queries on memory-optimized tables are compiled and executed, including:  
  
-   The query processing pipeline in [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)] for disk-based tables.  
...
-   The use of interpreted [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] to access memory-optimized tables.  
...
## Example Query  
 ...
 We consider two tables, Customer and Order. The following [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] script contains the definitions for these two tables and associated indexes, in their (traditional) disk-based form:  
...
```  
 ...
 The estimated execution plan as displayed by [!INCLUDE[ssManStudioFull](../../a9notintoc/includes/ssmanstudiofull-md.md)] is as follows  
...
## [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)] Query Processing for Disk-Based Tables  
 The following diagram outlines the query processing flow in [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)] for ad hoc queries:  
...
2.  The parser and algebrizer construct a query tree with logical operators based on the [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] text submitted by the user.  
...
## Interpreted [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] Access to Memory-Optimized Tables  
 [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] ad hoc batches and stored procedures are also referred to as interpreted [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)]. Interpreted refers to the fact that the query plan is interpreted by the query execution engine for each operator in the query plan. The execution engine reads the operator and its parameters and performs the operation.  
  
 Interpreted [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] can be used to access both memory-optimized and disk-based tables. The following figure illustrates query processing for interpreted [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] access to memory-optimized tables:  
...
-   The query execution engine interprets the execution plan.  
 ...
 The following [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] script contains memory-optimized versions of the Order and Customer tables, using hash indexes:  
...
1.  The user issues a **CREATE PROCEDURE** statement to [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)].  
  
2.  The parser and algebrizer create the processing flow for the procedure, as well as query trees for the [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] queries in the stored procedure.  
...
### Retrieving a Query Execution Plan for Natively Compiled Stored Procedures  
 The query execution plan for a natively compiled stored procedure can be retrieved using **Estimated Execution Plan** in [!INCLUDE[ssManStudio](../../a9notintoc/includes/ssmanstudio-md.md)], or using the SHOWPLAN_XML option in [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)]. For example:  
...
|Nested Loops Join|`SELECT o.OrderID, c.CustomerID FROM dbo.[Order] o INNER JOIN dbo.[Customer] c`|Nested Loops is the only join operator supported in natively compiled stored procedures. All plans that contain joins will use the Nested Loops operator, even if the plan for same query executed as interpreted [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] contains a hash or merge join.|  
...
-   The Merge Join operator has estimated 815; actual 830.  
 ...
 The estimates for the index scans are accurate. [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)] maintains the row count for disk-based tables. Estimates for full table and index scans are always accurate. The estimate for the join is fairly accurate, too.  
