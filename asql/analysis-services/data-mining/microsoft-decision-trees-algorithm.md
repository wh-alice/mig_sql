...
# Microsoft Decision Trees Algorithm
  The [!INCLUDE[msCoName](../../a9notintoc/includes/msconame-md.md)] Decision Trees algorithm is a classification and regression algorithm for use in predictive modeling of both discrete and continuous attributes.  
...
### Predicting Discrete Columns  
 ...
 The correlation that is shown in the diagram would cause the [!INCLUDE[msCoName](../../a9notintoc/includes/msconame-md.md)] Decision Trees algorithm to create a new node in the model.  
...
### Predicting Continuous Columns  
 When the [!INCLUDE[msCoName](../../a9notintoc/includes/msconame-md.md)] Decision Trees algorithm builds a tree based on a continuous predictable column, each node contains a regression formula. A split occurs at a point of non-linearity in the regression formula. For example, consider the following diagram.  
 ...
 In a standard regression model, you would attempt to derive a single formula that represents the trend and relationships for the data as a whole.  However, a single formula might do a poor job of capturing the discontinuity in complex data. Instead, the [!INCLUDE[msCoName](../../a9notintoc/includes/msconame-md.md)] Decision Trees algorithm looks for segments of the tree that are largely linear and creates separate formulas for these segments. By breaking up the data into different segments, the model can do a much better job of approximating the data.  
