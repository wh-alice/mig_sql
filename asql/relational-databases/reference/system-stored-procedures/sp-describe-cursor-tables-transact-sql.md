...
## Cursors Returned  
 sp_describe_cursor_tables encapsulates its report as a [!INCLUDE[tsql](../../../a9notintoc/includes/tsql-md.md)] **cursor** output parameter. This enables [!INCLUDE[tsql](../../../a9notintoc/includes/tsql-md.md)] batches, stored procedures, and triggers to work with the output one row at a time. This also means that the procedure cannot be called directly from API functions. The **cursor** output parameter must be bound to a program variable, but the APIs do not support bind **cursor** parameters or variables.  
...
|Table_name|**sysname**|Name of the object or base table. In [!INCLUDE[ssNoVersion](../../../a9notintoc/includes/ssnoversion-md.md)], server cursors always return the user-specified object, not the base tables.|  
...
## Remarks  
 sp_describe_cursor_tables describes the base tables that are referenced by a server cursor. For a description of the attributes of the result set returned by the cursor, use sp_describe_cursor_columns. For a description of the global characteristics of the cursor, such as its scrollability and updatability, use sp_describe_cursor. To obtain a report of the [!INCLUDE[tsql](../../../a9notintoc/includes/tsql-md.md)] server cursors that are visible on the connection, use sp_cursor_list.  
