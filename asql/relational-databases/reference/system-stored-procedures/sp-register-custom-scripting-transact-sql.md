...
|**custom_script**|Script is executed at the end of the data definition language (DDL) trigger.|  
 ...
 [ **@value**= ] **'***value***'**  
 Name of a stored procedure or name and fully-qualified path to the [!INCLUDE[tsql](../../../a9notintoc/includes/tsql-md.md)] script file that is being registered. *value* is **nvarchar(1024)**, with no default.  
...
>  Specifying NULL for *value*parameter will unregister a previously registered script, which is the same as running [sp_unregister_custom_scripting](../../../relational-databases/reference/system-stored-procedures/sp-unregister-custom-scripting-transact-sql.md).  
 ...
 When the value of *type* is **custom_script**, the name and full path of a [!INCLUDE[tsql](../../../a9notintoc/includes/tsql-md.md)] script file is expected. Otherwise, *value* must be the name of a registered stored procedure.  
