...
|**subscriber_security_mode**|**smallint**|Is the security mode at the Subscriber, where **1** means Windows Authentication, and **0** means [!INCLUDE[msCoName](../../../advanced-analytics/r-services/tutorials/includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../advanced-analytics/r-services/includes/ssnoversion-md.md)] Authentication.|  
 ...
 **sp_helpmergesubscription** is used in merge replication to return subscription information stored at the Publisher or republishing Subscriber.  
 ...
 For anonymous subscriptions, the *subscription_type*value is always **1** (pull). However, you must execute [sp_helpmergepullsubscription](../../../relational-databases/reference/system-stored-procedures/sp-helpmergepullsubscription-transact-sql.md) at the Subscriber for information on anonymous subscriptions.  
