...
|**Type**|Node type. For the parent node of each query, this is the [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] statement type (for example, SELECT, INSERT, EXECUTE, and so on). For subnodes representing execution plans, the type is PLAN_ROW.|  
...
## Examples  
 The two statements that follow use the SET SHOWPLAN_ALL settings to show the way [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)] analyzes and optimizes the use of indexes in queries.  
