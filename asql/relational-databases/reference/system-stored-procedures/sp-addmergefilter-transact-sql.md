...
|Value|Description|  
|-----------|-----------------|  
|**1**|Join filter only. Required to support [!INCLUDE[ssEW](../../../analysis-services/instances/includes/ssew-md.md)] Subscribers.|  
 ...
 **sp_addmergefilter** can only be used with table articles. View and indexed view articles are not supported.  
 ...
 When both a parameterized row filter and a join filter are used on a table article, replication determines whether a row belongs in a Subscriber's partition. It does so by evaluating either the filtering function or the join filter (using the [OR](../Topic/OR%20\(Transact-SQL\).md) operator), rather than evaluating the intersection of the two conditions (using the [AND](../Topic/AND%20\(Transact-SQL\).md) operator).  
  
## Example  
 [!code-sql[HowTo#sp_addmergefilter](../../../relational-databases/reference/system-stored-procedures/codesnippet/tsql/sp-addmergefilter-transa_1.sql)]  
