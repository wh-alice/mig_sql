...
## Types of DDL Triggers  
 ...
 A special type of [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] stored procedure that executes one more more [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] statements in response to a server-scoped or database-scoped event. For example, a DDL Trigger may fire if a statement such as ALTER SERVER CONFIGURATION is executed or if a table is deleted by using DROP TABLE.  
  
 CLR DDL Trigger  
 Instead of executing a [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] stored procedure, a CLR trigger executes one or more methods written in managed code that are members of an assembly created in the .NET Framework and uploaded in [!INCLUDE[ssNoVersion](../../a9notintoc/includes/ssnoversion-md.md)].  
...
```  
 ...
 The lists that map the [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] statements to the scopes that can be specified for them are available through the links provided in the section "Selecting a Particular DDL Statement to Fire a DDL Trigger," later in this topic.  
