...
|**offload_server**|**sysname**|Name of the server enabled for remote agent activation. If NULL, then the current offload_server listed in [MSdistribution_agents](../../../relational-databases/reference/system-tables/msdistribution-agents-transact-sql.md) table is used.|  
...
|**subscriber_security_mode**|**smallint**|Is the security mode at the Subscriber, where **1** means Windows Authentication, and **0** means [!INCLUDE[ssNoVersion](../../../advanced-analytics/r-services/includes/ssnoversion-md.md)] Authentication.|  
