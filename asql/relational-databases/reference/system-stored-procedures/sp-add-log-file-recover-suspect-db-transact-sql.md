...
## Arguments  
 ...
 [ **@name=** ] **'***logical_file_name***'**  
 Is the name used in the [!INCLUDE[ssNoVersion](../../../a9notintoc/includes/ssnoversion-md.md)] when referencing the file. The name must be unique in the server. *logical_file_name* is **nvarchar(260)**, with no default.  
  
 [ **@filename =** ] **'***os_file_name***'**  
 Is the path and file name used by the operating system for the file. The file must reside in the server in which the [!INCLUDE[ssDE](../../../a9notintoc/includes/ssde-md.md)] is installed. *os_file_name* is **nvarchar(260)**, with no default.  
 ...
 If *max_size* is not specified, the file will grow until the disk is full. The [!INCLUDE[msCoName](../../../a9notintoc/includes/msconame-md.md)] Windows application log warns an administrator when a disk is about to become full.  
