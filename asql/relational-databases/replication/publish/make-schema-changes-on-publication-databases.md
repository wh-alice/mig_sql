...
-   ALTER TABLE SET LOCK ESCALATION should not be used if schema change replication is enabled and a topology includes [!INCLUDE[ssVersion2005](../../../a9notintoc/includes/ssversion2005-md.md)] or [!INCLUDE[ssEWnoversion](../../../relational-databases/replication/includes/ssewnoversion-md.md)] Subscribers.ALTER VIEW  
...
-   Schema changes are subject to any restrictions imposed by [!INCLUDE[tsql](../../../a9notintoc/includes/tsql-md.md)]. For example, ALTER TABLE does not allow you to ALTER primary key columns.  
  
-   Data type mapping is performed only for the initial snapshot. Schema changes are not mapped to previous versions of data types. For example, if the statement `ALTER TABLE ADD datetime2 column` is used in [!INCLUDE[ssSQL11](../../../a9notintoc/includes/sssql11-md.md)], the data type is not translated to **nvarchar** for [!INCLUDE[ssVersion2005](../../../a9notintoc/includes/ssversion2005-md.md)] Subscribers. In some cases, schema changes are blocked on the Publisher.  
...
-   Schema changes are propagated to Subscribers running previous versions of [!INCLUDE[ssNoVersion](../../../a9notintoc/includes/ssnoversion-md.md)], but the DDL statement should only include syntax supported by the version at the Subscriber.  
...
-   Schema changes are not propagated from non-[!INCLUDE[ssNoVersion](../../../a9notintoc/includes/ssnoversion-md.md)] Publishers.  
...
-   How merge replication handles schema changes is determined by the publication compatibility level, and whether the snapshot is set to native mode (default) or character mode:  
    ...
    -   If you try to add to an existing article a column with a data type that was introduced in [!INCLUDE[ssKatmai](../../../a9notintoc/includes/sskatmai-md.md)], [!INCLUDE[ssNoVersion](../../../a9notintoc/includes/ssnoversion-md.md)] has the following behavior:  
