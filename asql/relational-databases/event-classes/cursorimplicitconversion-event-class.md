...
---
# CursorImplicitConversion Event Class
  The **CursorImplicitConversion** event class describes cursor-implicit conversion events that occur in application programming interfaces (APIs) or [!INCLUDE[tsql](../../a9notintoc/includes/tsql-md.md)] cursors. Cursor implicit conversion events occur when the [!INCLUDE[ssDEnoversion](../../a9notintoc/includes/ssdenoversion-md.md)] executes a Transact-SQL statement that is not supported by server cursors of the type requested. The [!INCLUDE[ssDE](../../a9notintoc/includes/ssde-md.md)] returns an error that indicates the cursor type has changed.  
